...
import { DragDropContext, Droppable, Draggable } from '@hello-pangea/dnd';const SoundBoardPage = ({ soundDatabase, selectedBoardFromSidebar, onBoardChange }) => {
...
} = useSoundBoard();  const [selectedBoard, setSelectedBoard] = useState(null);  React.useEffect(() => {
...
const audioRef = useRef(null);  const playSound = (sound) => {
if (audioRef.current) {
...
addToSoundBoardHistory(sound);    audio.play().catch(err => console.log('Erreur lecture:', err));
audio.onended = () => {
setCurrentlyPlaying(null);      if (audio.parentNode) {
...
if (!result.destination || !selectedBoard) return;    const newSounds = [...selectedBoard.sounds];
const [removed] = newSounds.splice(result.source.index, 1);
newSounds.splice(result.destination.index, 0, removed);    const updatedBoard = { ...selectedBoard, sounds: newSounds };
...
</div>          {/* Contrôle de volume global des sons */}
...
style={{ flex: 1, cursor: 'pointer' }}
/>
</div>          {/* Liste des sons du soundboard */}
...
const { isDarkMode, currentTheme } = useTheme();  return (
<div className="modal-backdrop" onClick={onClose}>
